...
def gen_beam_output_file(c):
    ...
    This function processes all configured episodes or imported H-matrix data
    to compute MIMO channel matrices, beam indices, and equivalent channel
    magnitudes. The generated arrays are stacked and saved as compressed NumPy
    ``.npz`` files in the beam output directory.

    The function supports two modes:
    - importing H-matrix CSV files generated by Wireless InSite;
    - computing MIMO channels from ray data stored in HDF5 files.

    Args:
        c: Runtime configuration object containing simulation paths, run range,
            episode settings, MIMO/codebook options, and output names.

    Returns:
        None. The generated H-matrix, beam index, and channel arrays are saved
        to disk.

    Raises:
        FileNotFoundError: If required input folders or files are missing.
        OSError: If an input HDF5 file cannot be opened.
        ValueError: If the generated arrays cannot be stacked due to inconsistent
            shapes.
    """
